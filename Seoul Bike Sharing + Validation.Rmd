...
author: "Wan Ying Hsu; Margarita Shebanova; Chueh Yang; Elvina Chen-Hsin Yang"
date: "10/6/2021"
output: 
  html_document:
    toc_depth: 4
...
<center>
![Seoul Bike Sharing](D:/HSLU/2021_Spring/2_W.MSCIDS_MPM02.F2101_Applied Machine Learning and Predictive Modelling 1/Group work _all/SeoulBike.jpeg)
...
bikes <- read.csv("SeoulBikeData_Original.csv", header = TRUE, fileEncoding = "Latin1", check.names = F)
library(pander)
...
```
Last, we separated the dataset into 80% for train models and 20% to test models randomly by hour.
...
```
**Conclusion:** From the residual analysis graphs, a long-tail effect on the left side in the QQ plot. While the in-sample R-Square was 78%, it dropped into 70% in out-sample test. A linear model doesn't really perfectly fit the data.
...
#capture.output(summary(gam.0))[24:29]
summary(gam.0)
```
...
glm.full2 <- glm(bike_count ~ month + hour + poly(temperature, degree=2) + wind_speed + 
                visibility + dew + solar + rainfall + snowfall, family = "poisson", data = train.p)
summary(glm.full2)
...
```{r include=TRUE, message=FALSE, warning=FALSE}
#pred.bi <- predict(glm_bike2, type = "response", newdata = test)
...
From the graphic analysis section, it seems that high-demand factor shows difference in all variables. Therefore, we'll put them into the svm model.
...
```
In the test data using the svm model, 739 observations were correctly labeled to low-demand and 813 were correctly labeled to high-demand. Meanwhile, 70 low-demand observations were falsely classified to be high-demand in this model, while 58 high-demand ones to be low-demand. Overall, the model has 92% accuracy. 

# Neural Network
## Data Understanding

Read necessary library and read the dataset. Also, look at the structure of the dataset. 
...
Delete variables which won't be used in the modeling process and set factors. And inspect the dataset before building a model.
...
Since the model has pretty high accuracy, the curve shows an absolute pattern - with a high true positive rate and very low false positive rate. 


# Cross Validation

To split the data into training and testing datasets, we used 20-fold (80% of the data are used to train the model and 20% to test it). 
We aim at finding a model that maximises the predictive performance of demand for bikes rentals. Three models - Linear Model, Generalised Additive Model (GAM), GLM with Poisson - predict the *bike_count* variable. 
We used **R-squared** as a measure of fit because of the three following reasons: (1) ease of computation, (2) it makes sure we have less extreme outliers, (3) it is symmetric. The last advantage is significant in our case because over- or underestimating demand for bikes rentals is considered to be equally bad.

The R-squared values are provided in the table below.

```{r}
R.df <- data.frame(Linear_Model = round(R.lm,3), GAM = round(R.gam,3), GLM_Poisson = round(R.glm3, 3), 
                   GLM_Quasipoisson = round(R.glmq, 3), GLM_Negative_Binomial = round(R.glmnb, 3))
R.df
```

Three other models - GLM with Binomial, Support Vector Machine, Neural Network - predict the binary *highdemand* variable. As a measure of predictive performance for these models, we used the **accuracy** value from a confusion matrix (the degree of closeness to actual value).
